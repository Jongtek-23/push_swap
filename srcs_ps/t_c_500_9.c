...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/05/11 00:56:06 by focampo           #+#    #+#             */
/*   Updated: 2021/05/11 00:56:07 by focampo          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../include/push_swap.h"

int	tmp_500_value_petit_9(t_all *all)
{
	int	nbr;
	int	len;
	int	i;
...
int	tmp_500_value_grand_9(t_all *all)
{
	int	nbr;
	int	len;
	int	i;
...
void	t_c_500_9_petit(t_all *all, t_list **stack_a, t_list **stack_b)
{
	...
	while (all->boucle > 0)
	{
		...
		all->div = len_b / 2;
		all->value_choose = tmp_500_value_petit_9(all);
...
void	t_c_500_9_grand(t_all *all, t_list **stack_a, t_list **stack_b)
{
	...
	while (all->boucle > 0)
	{
		len_b = ft_lstsize(*stack_b);
		all->div = len_b / 2;
		all->value_choose = tmp_500_value_grand_9(all);
		all->pos_in_stack_b = get_position_val(*stack_b, all->value_choose);
		all->dist_top_stack_b_up = all->pos_in_stack_b - 1;
		all->dist_top_stack_b_back = len_b - all->pos_in_stack_b + 1;
		move_value_top_b(all, stack_b);
