...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/05/10 10:52:44 by focampo           #+#    #+#             */
/*   Updated: 2021/05/10 10:52:45 by focampo          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../include/push_swap.h"

int	get_tmp_value_petit_2(t_all *all)
{
	int	nbr;
	int	len;
	int	i;
...
int	get_tmp_value_grand_2(t_all *all)
{
	int	nbr;
	int	len;
	int	i;
...
void	trier_chunk_2_petit(t_all *all, t_list **stack_a, t_list **stack_b)
{
	...
	while (all->boucle > 0)
	{
		...
		all->div = len_b / 2;
		all->value_choose = get_tmp_value_petit_2(all);
...
void	trier_chunk_2_grand(t_all *all, t_list **stack_a, t_list **stack_b)
{
	...
	while (all->boucle > 0)
	{
		len_b = ft_lstsize(*stack_b);
		all->div = len_b / 2;
		all->value_choose = get_tmp_value_grand_2(all);
		all->pos_in_stack_b = get_position_val(*stack_b, all->value_choose);
		all->dist_top_stack_b_up = all->pos_in_stack_b - 1;
		all->dist_top_stack_b_back = len_b - all->pos_in_stack_b + 1;
		move_value_top_b(all, stack_b);
