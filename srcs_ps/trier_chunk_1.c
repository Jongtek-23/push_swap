...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/05/10 10:52:36 by focampo           #+#    #+#             */
/*   Updated: 2021/05/10 10:52:38 by focampo          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../include/push_swap.h"

int	get_tmp_value_petit_1(t_all *all)
{
	int	nbr;
	int	len;
	int	i;
...
int	get_tmp_value_grand_1(t_all *all)
{
	int	nbr;
	int	len;
	int	i;
...
void	trier_chunk_1_petit(t_all *all, t_list **stack_a, t_list **stack_b)
{
	...
	while (all->boucle > 0)
	{
		...
		all->div = len_b / 2;
		all->value_choose = get_tmp_value_petit_1(all);
...
void	trier_chunk_1_grand(t_all *all, t_list **stack_a, t_list **stack_b)
{
	...
	while (all->boucle > 0)
	{
		len_b = ft_lstsize(*stack_b);
		all->div = len_b / 2;
		all->value_choose = get_tmp_value_grand_1(all);
		all->pos_in_stack_b = get_position_val(*stack_b, all->value_choose);
		all->dist_top_stack_b_up = all->pos_in_stack_b - 1;
		all->dist_top_stack_b_back = len_b - all->pos_in_stack_b + 1;
		move_value_top_b(all, stack_b);
